...
func (h *userHandler) OtpRequest(c *gin.Context) {
	...
	response := helper.APIResponse("OTP has been sent to your email", http.StatusOK, "success", user.FormatOTP(newOtpRequest))

	c.JSON(http.StatusOK, response)
}

func (h *userHandler) UpdateIsVerifiedOTP(c *gin.Context) {
	var inputData user.OTPUserInput
	...
	if err != nil {
		...
		response := helper.APIResponse("Failed to update is verified", http.StatusUnprocessableEntity, "error", errorMessage)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}

	updatedIsVerified, err := h.userService.UpdateIsVerifiedOTP(inputData)
	if err != nil {
		response := helper.APIResponse("Failed to update is verified", http.StatusBadRequest, "error", nil)
		c.JSON(http.StatusBadRequest, response)
		return
	}

	response := helper.APIResponse("Success to update is verified", http.StatusOK, "success", user.FormatOTP(updatedIsVerified))
	c.JSON(http.StatusOK, response)
}
